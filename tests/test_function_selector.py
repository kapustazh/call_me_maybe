...
def test_selects_from_logits_among_many_candidates() -> None:
    class BiasedAddModel:
        def encode(self, text: str) -> list[list[int]]:
            return [[ord(ch) for ch in text]]
    ...
    selector = FunctionSelector(
        cast(Small_LLM_Model, BiasedAddModel()),
        many,
    )
...
def test_load_functions_definition_json_file() -> None:
    root = Path(__file__).resolve().parents[1]
    path = root / "data_test/input/functions_definition.json"
    defs = load_function_definitions(path)
    assert len(defs) == 13
    assert defs[0].name == "fn_add_numbers"
    assert defs[0].parameters["a"].type == "integer"
    assert defs[6].name == "fn_is_even"
    assert defs[6].parameters["n"].type == "integer"
